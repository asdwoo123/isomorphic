...
class DndCalendar extends Component {
  ...
  saveOnEvents = () => {
    ...
    firebase.firestore().collection('calendar').doc(localStorage.getItem('user_id'))
        .set({ events: ['das', 'asd'] }).then(() => {
  ...
  render() {
    return (
        <>
        <button type="button" onClick={this.saveOnEvents}>Save</button>
