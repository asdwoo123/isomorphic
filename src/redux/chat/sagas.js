...
const sendMessageBatch = async payload => {
  ...
  const { chatRoom, text, composedId } = payload;
  const userId = fsProps.userId;
  let id;
  if (!chatRoom) {
    const chatRoomRef = database.collection('chatRooms').doc();
    database.collection('users').doc(composedId).get().then((doc) => {
      if (doc.exists) {
        const { name, profileImageUrl } = doc.data();
        chatRoomRef.set({
          id: chatRoomRef.id,
          lastMessage: '',
          lastMessageTime: 0,
          otherUserId: composedId,
          userId,
          otherUserInfo: {
            id: composedId,
            name,
